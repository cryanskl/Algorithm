...
<img src="https://wuzhi-img.oss-cn-shanghai.aliyuncs.com/img/image-20220207153407104.png" alt="image-20220207153407104" style="zoom:50%;" />
...
- **深拷贝：**深拷贝相对浅拷贝而言，如果遇到属性值为引用类型的时候，它新建一个引用类型并将对应的值复制给它，因此对象获得的一个新的引用类型而不是一个原有类型的引用(即不会改变原有引用)。深拷贝对于一些对象可以使用 JSON 的两个函数来实现，但是由于 JSON 的对象格式比 js 的对象格式更加严格，所以如果属性值里边出现函数, undefined 或者 Symbol 类型的值时，会转换失败
...
### 16. 创建 10 个`<a>`标签, 点击的时候弹出对应的序号

<img src="https://wuzhi-img.oss-cn-shanghai.aliyuncs.com/img/image-20220204010450968.png" alt="image-20220204010450968" style="zoom:50%;" />
